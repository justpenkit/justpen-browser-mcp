...
- **Parallel logged-in sessions** — one instance per user / tenant, no cookie
    bleed.
- **Clean-slate flows** — use an ephemeral instance and destroy it after a
    test to discard its session state. A persistent `profile_dir` survives
    destruction; choose a fresh profile directory when you need a new session.
...
codes](response-envelope.md#error_type-values)) instead of operating
on a dead browser. Until that next call happens, [`browser_list_instances`](../tools-reference/lifecycle.md#browser_list_instances)
and [`browser_health`](../tools-reference/lifecycle.md#browser_health) will show
the instance with `status: "crashed"`.

[`browser_destroy_instance`](../tools-reference/lifecycle.md#browser_destroy_instance)
...
- [`browser_health`](../tools-reference/lifecycle.md#browser_health) — check server status without launching a browser.

## Single active page assumption { #single-active-page-assumption }
