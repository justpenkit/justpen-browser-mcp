...
This browser server adopted the shared development framework through a reviewed
migration. Its Python package, tools, release history and product documentation
remain application-owned. To connect Codex as a browser-tool consumer, see
[Codex client setup](../client-setup/codex.md).

## First setup

Run `make setup` in a terminal before starting either agent. The development
workflow supports Apple silicon macOS, Linux, and 64-bit Windows with WSL, uv,
Git and Make. uv manages Python
...
`make setup` also fetches the Camoufox browser binary needed for end-to-end tests.
...
exact configuration: review again when it changes. The project does not edit
