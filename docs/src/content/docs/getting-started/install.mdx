...
`uv` resolves the `playwright>=1.49,<1.60` pin from `pyproject.toml`
automatically — Camoufox 0.4.x's Firefox build is incompatible with
Playwright ≥1.60, so you don't need to do anything, but if you ever see a
version conflict here, don't widen the pin by hand.
