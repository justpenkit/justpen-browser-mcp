...
description: Pre-PR contributor checklist.
---
...
make docs-build  # astro build (no built-in link check; CI runs lychee for that)
```
...
also run `make docs-build`. Broken internal links are caught by CI's
`lychee` step.
