...
**Returns** — see [response envelope](/concepts/response-envelope/). `data` shape (mutually exclusive):
...
**Notes** — The dialog must already be open before calling this tool; it was triggered by a prior tool call and captured automatically by the modal-state listener. This tool does not pre-register a handler for future dialogs. See [Modal state](/concepts/modal-state/) for the broader dialog lifecycle.
