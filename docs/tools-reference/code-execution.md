...
```python
async def browser_evaluate(context: str, expression: str, ref: str | None = None, selector: str | None = None) -> dict[str, Any]
```
...
```json
{ "name": "browser_evaluate", "arguments": { "context": "main", "expression": "document.title" } }
```

Response:

```json
{ "status": "success", "context": "main", "data": { "result": "Example Domain" } }
```
...
```json
{ "status": "success", "context": "main", "data": { "result": "Task Complete" } }
```

**Notes** — The snippet runs with `page`, `context` (the Playwright BrowserContext object), and `ctx_mgr` (ContextManager, for advanced use) in scope. Any exception raised is caught and returned as `evaluation_failed` with the original traceback included in the message.
