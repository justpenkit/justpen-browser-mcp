"""Parse browser-scoped settings without inheriting the harness's OTel configuration."""
...
def read_config(env: Mapping[str, str]) -> TelemetryConfig:
    """Read only browser-scoped settings and the authoritative session variable."""
    ...
    return TelemetryConfig(
        ...
        service_name=env.get(PREFIX + "SERVICE_NAME") or attributes.get("service.name") or "justpen-browser-mcp",
        resource_attributes=MappingProxyType(attributes),
        sdk_environment=MappingProxyType(sdk_environment),
        require_session=_boolean(env, PREFIX + "REQUIRE_SESSION", default=False),
