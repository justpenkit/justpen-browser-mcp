"""Enrich FastMCP's native request span without creating or reparenting spans."""
...
from .payloads import apply_tool_result, known_method, safe_attributes
...
def _request_attributes(context: MiddlewareContext[Any]) -> dict[str, AttributeValue]:
    ...
    if (ctx := context.fastmcp_context) is not None:
        ...
        try:
            ...
            # Initialize/early rejection may have no established session yet.
            pass
...
class TelemetryMiddleware(Middleware):
    ...
    async def on_request(self, context: MiddlewareContext[Any], call_next: CallNext[Any, Any]) -> Any:
        """Own the observation and terminal event within the existing native span."""
    ...
    async def on_call_tool(self, context: MiddlewareContext[Any], call_next: CallNext[Any, ToolResult]) -> ToolResult:
        """Interpret only the final result returned by the operation middleware."""
