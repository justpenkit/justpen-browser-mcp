"""Preserve Browser documentation contracts alongside the shared MkDocs tests."""
...
def documentation_project(tmp_path):
    if sys.version_info[:2] != (3, 13):
        pytest.skip("Documentation builds are validated on Python 3.13")
...
def test_tool_reference_matches_registered_browser_tools():
    documented = set()
    pages = sorted((ROOT / "docs" / "tools-reference").glob("*.md"))
    assert len(pages) == 10
    for page in pages:
        documented.update(re.findall(r"^## (browser_\w+)", page.read_text(), flags=re.MULTILINE))
    registered = set()
...
@pytest.mark.integration
def test_tool_reference_renders_tool_anchors(built_documentation):
    for page in sorted((ROOT / "docs" / "tools-reference").glob("*.md")):
