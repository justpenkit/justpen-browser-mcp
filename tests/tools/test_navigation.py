...
class TestBrowserNavigate:
    async def test_success(self, mcp_client, mock_ctx_mgr):
        page = make_page(
            mock_ctx_mgr, url="https://app.example.com/dashboard", title="Dashboard"
        )
        ...
        page.goto.assert_awaited_once_with(
            "https://app.example.com/dashboard", wait_until="domcontentloaded"
        )
...
class TestBrowserNavigateBack:
    async def test_success(self, mcp_client, mock_ctx_mgr):
        page = make_page(mock_ctx_mgr)
        result = await mcp_client.call_tool(
            "browser_navigate_back", {"context": "admin"}
        )
        assert result.data["status"] == "success"
...
class TestBrowserWaitFor:
    async def test_text_visible(self, mcp_client, mock_ctx_mgr):
        page = make_page(mock_ctx_mgr)
        result = await mcp_client.call_tool(
            "browser_wait_for", {"context": "admin", "text": "Welcome"}
        )
        assert result.data["status"] == "success"
        page.get_by_text.assert_called_once_with("Welcome")
        page.get_by_text.return_value.first.wait_for.assert_awaited_once_with(
            state="visible"
        )
    ...
    async def test_text_gone(self, mcp_client, mock_ctx_mgr):
        page = make_page(mock_ctx_mgr)
        result = await mcp_client.call_tool(
            "browser_wait_for", {"context": "admin", "text_gone": "Loading"}
        )
        assert result.data["status"] == "success"
    ...
    async def test_time_in_seconds(self, mcp_client, mock_ctx_mgr):
        page = make_page(mock_ctx_mgr)
        result = await mcp_client.call_tool(
            "browser_wait_for", {"context": "admin", "time": 0.5}
        )
        assert result.data["status"] == "success"
    ...
    async def test_time_capped_at_30s(self, mcp_client, mock_ctx_mgr):
        page = make_page(mock_ctx_mgr)
        result = await mcp_client.call_tool(
            "browser_wait_for", {"context": "admin", "time": 60}
        )
        assert result.data["status"] == "success"
