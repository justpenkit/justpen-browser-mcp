...
class WireServer:
    def __init__(
        self, tmp_path, collector, *, transport="stdio", stateless=False, env=None, client_name="fixture-client"
    ):
        ...
        self.env = {
            ...
            if not name.startswith(("OTEL_", PREFIX, "FASTMCP_", "BROWSER_MCP_")) and name != "JUSTPEN_SESSION_ID"
        }
        self.env.update(
            {
                PREFIX + "ENABLED": "true",
                ...
                "BROWSER_MCP_OPERATION_TIMEOUT_SECONDS": "5",
                "BROWSER_MCP_MAX_RESULT_BYTES": "4096",
    ...
    async def call(self, mode="success", label="a", **kwargs):
        return await self.request(
            "tools/call", {"name": "browser_telemetry_probe", "arguments": {"mode": mode, "label": label}}, **kwargs
        )
    ...
    async def wait_for_running(self, label):
        deadline = time.monotonic() + 3
        while time.monotonic() < deadline:
            response = await self.request("tools/call", {"name": "browser_telemetry_running"})
            running = response["result"]["structuredContent"]["data"]["running"]
            if label in running:
                return running[label]
            await asyncio.sleep(0.01)
        raise AssertionError("Blocking fixture never entered its operation")
